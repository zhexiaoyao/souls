...
void WeaponModel::CreateWeapon(ObjectManager* objectManager) {
    ...
    // Scale along Z axis (forward direction) to make weapon point forward
    ...
    body->SetScale(0.3f, 0.3f, 1.5f);  // Z is length (forward), X and Y are width/height
    
    // Create barrel (thin cylinder) - positioned forward along Z axis
    ...
    barrel->SetPosition(0.0f, 0.0f, 1.0f);  // Position forward (Z axis) instead of X axis
    barrel->SetRotation(90.0f, 0.0f, 0.0f);  // Rotate 90 degrees around X axis to make cylinder horizontal (pointing forward)
    
    // Create stock (cuboid) - positioned backward along Z axis
    ...
    stock->SetPosition(0.0f, 0.0f, -0.4f);  // Position backward (negative Z) instead of negative X
    stock->SetScale(0.3f, 0.4f, 0.8f);  // Adjusted: Z is depth, Y is height, X is width
    
    // Create scope base (larger cylinder for scope body) - positioned on top, forward
    ...
    scopeBase->SetPosition(0.0f, 0.2f, 0.4f);  // Y up, Z forward (instead of X forward, Y up)
    scopeBase->SetRotation(90.0f, 0.0f, 0.0f);  // Rotate 90 degrees around X to make horizontal (forward)
    ...
    scopeRing->SetPosition(0.0f, 0.2f, 0.4f);  // Same position as scope base
    scopeRing->SetRotation(90.0f, 0.0f, 0.0f);  // Rotate 90 degrees around X to make horizontal
    ...
    scopeLens->SetPosition(0.0f, 0.2f, 0.4f);  // Same position as scope base
    scopeLens->SetRotation(90.0f, 0.0f, 0.0f);  // Rotate 90 degrees around X to make horizontal
